...
from mir_ci.protocols.wlr_virtual_pointer_unstable_v1.zwlr_virtual_pointer_manager_v1 import ZwlrVirtualPointerManagerV1Proxy
...
class VirtualPointer(WaylandClient):
    ...
    def registry_global(self, registry, id_num: int, iface_name: str, version: int) -> None:
        ...
        if iface_name == ZxdgOutputManagerV1.name:
            self.output_manager = registry.bind(
                id_num,
                ZxdgOutputManagerV1,
                min(ZxdgOutputManagerV1.version, version))
        if iface_name == WlOutput.name:
            self.wl_outputs.append(registry.bind(
                id_num,
                WlOutput,
                min(WlOutput.version, version)))
